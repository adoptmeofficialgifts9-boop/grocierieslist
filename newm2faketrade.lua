...
local function OnItemsChanged()
    RefreshAllSlots()
    if SimState == "Accepted" then pipelineEpoch += 1; SimState="Active"; AcceptState="Accept"; resetTradeUI() end
    ...
    local yc, tc = 0, 0
    for i = 1, MAX_SLOTS do
        if YourSlots[i]  then yc += 1 end
        if TheirSlots[i] then tc += 1 end
    end
...
SetSimState = function(newState)
    ...
    elseif newState == "Accepted" then
        AcceptState = "Waiting"
        RebuildTradeTable(); TradeTable.Player1.Accepted = true
        ...
        task.delay(1.4, function()
            ...
            task.delay(0.6, function()
                ...
                local function GiveItem(itemID, itemType, amount)
                    pcall(function()
                        ...
                        if ProfileData and ProfileData[itemType] and ProfileData[itemType].Owned then
                            ...
                            ProfileData[itemType].Owned[itemID] = (cur or 0) + amount
                        end
                ...
                for _, item in ipairs(TradeTable.Player2.Offer) do GiveItem(item[1], item[3], item[2]) end
...
local function startFakeTrade(partnerName, theirItems)
    tradeGuiHooked = false; pipelineEpoch += 1
    ...
    for dbID, dbData in pairs(DB.Weapons) do
        if isValid(dbData) then
            ...
            if idMatch or nameMatch then
                ...
                if score > bestScore then bestScore=score; bestID=dbID; bestData=dbData end
            end
        end
    end
...
ScreenGui.Name="TradeManagerGui"; ScreenGui.ResetOnSpawn=false
...
MainFrame.Name="MainFrame"; MainFrame.Size=UDim2.new(0,250,0,520)
...
header.BackgroundColor3=Color3.fromRGB(255,182,210); header.BorderSizePixel=0; header.Parent=MainFrame
...
bowLabel.BackgroundTransparency=1; bowLabel.Text="\240\159\142\128"
...
headerTitle.BackgroundTransparency=1; headerTitle.Text="ZetaScripts \240\159\140\184"
...
CloseBtn.Text="\226\156\149"; CloseBtn.TextSize=12; CloseBtn.TextColor3=Color3.fromRGB(255,255,255)
...
local function createSectionLabel(text,parent)
    local lbl=Instance.new("TextLabel")
    lbl.Size=UDim2.new(1,0,0,15); lbl.BackgroundTransparency=1
    lbl.Text="\226\156\166 "..text; lbl.Font=Enum.Font.FredokaOne; lbl.TextSize=9
...
local function createToggleRow(labelText,defaultOn,parent,onChange)
    ...
    row.Size=UDim2.new(1,0,0,28); row.BackgroundTransparency=1; row.Active=true; row.Parent=parent
    local lbl=Instance.new("TextLabel")
    lbl.Size=UDim2.new(1,-56,1,0); lbl.BackgroundTransparency=1; lbl.Text=labelText
    lbl.Font=Enum.Font.FredokaOne; lbl.TextSize=11; lbl.TextColor3=C_LABEL
    lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Parent=row
    local isOn=defaultOn
    local c1=isOn and C_ON1 or C_OFF1
    local pill=Instance.new("TextButton")
    pill.Size=UDim2.new(0,50,0,22); pill.Position=UDim2.new(1,-52,0.5,-11)
    pill.BackgroundColor3=c1; pill.BackgroundTransparency=0.25
    pill.Text=isOn and "ON \226\156\147" or "OFF"
    pill.Font=Enum.Font.FredokaOne; pill.TextSize=10; pill.TextColor3=C_WHITE
    ...
    pillStroke.Color=c1:Lerp(C_WHITE,0.5); pillStroke.Thickness=1.5; pillStroke.Transparency=0.2; pillStroke.Parent=pill
    pill.MouseButton1Down:Connect(function()
        TweenService:Create(pill,TweenInfo.new(0.07,Enum.EasingStyle.Quad),{BackgroundTransparency=0.1}):Play()
        TweenService:Create(pillStroke,TweenInfo.new(0.07),{Transparency=0.0}):Play()
    end)
    local function pr()
        TweenService:Create(pill,TweenInfo.new(0.1,Enum.EasingStyle.Quad),{BackgroundTransparency=0.25}):Play()
        TweenService:Create(pillStroke,TweenInfo.new(0.1),{Transparency=0.2}):Play()
    end
    pill.MouseButton1Up:Connect(pr); pill.MouseLeave:Connect(pr)
    pill.MouseButton1Click:Connect(function()
        isOn=not isOn
        local nc=isOn and C_ON1 or C_OFF1
        pill.BackgroundColor3=nc
        pill.Text=isOn and "ON \226\156\147" or "OFF"
        pillStroke.Color=nc:Lerp(C_WHITE,0.5)
...
createFieldLabel("\240\159\146\140  PARTNER USERNAME",controlPanel)
...
local startBtn=createButton("\240\159\142\128  START FAKE TRADE",Color3.fromRGB(220,80,140),nil,nil,controlPanel)
local addTheirBtn=createButton("\240\159\140\184  ADD GODLY TO THEIR SIDE",Color3.fromRGB(100,180,130),nil,nil,controlPanel)
local removeTheirBtn=createButton("\226\156\130\239\184\143  REMOVE FROM THEIR SIDE",Color3.fromRGB(200,130,160),nil,nil,controlPanel)

createDivider(controlPanel)
createSectionLabel("TOOLS",controlPanel)

local blockBtn=createButton("\240\159\154\171  BLOCK PLAYER",Color3.fromRGB(220,60,90),nil,nil,controlPanel)
...
startBtn.MouseButton1Click:Connect(function()
    ...
    if partner=="" then showToast(statusToast,"\240\159\142\128  Enter a partner username first!",3); return end
    ...
    showToast(statusToast,"\240\159\140\184  Fake trade opened with "..partner,4)
...
addTheirBtn.MouseButton1Click:Connect(function()
    if partnerInput.Text=="" then showToast(statusToast,"\240\159\142\128  Enter a partner username first!",3); return end
    if fakeTrade.active then
        addGodlyToTheirSide()
        showToast(statusToast,"\240\159\140\184  Added godly to their side (live)",3)
    else
        local itemID=godlies[math.random(1,#godlies)]
        table.insert(stagedTheirItems,{ItemID=itemID,Amount=1,ItemType="Weapons"})
        showToast(statusToast,"\226\156\168  Staged '"..itemID.."' to their side",3)
    end
end)
...
removeTheirBtn.MouseButton1Click:Connect(function()
    if fakeTrade.active then
        removeFromTheirSide()
        showToast(statusToast,"\226\156\130\239\184\143  Removed from their side (live)",3)
    elseif #stagedTheirItems==0 then
        showToast(statusToast,"\240\159\142\128  No staged items to remove.",3)
    else
        local last=table.remove(stagedTheirItems)
        showToast(statusToast,"\226\156\130\239\184\143  Removed '"..last.ItemID.."'",3)
    end
end)

blockBtn.MouseButton1Click:Connect(function()
    local name=partnerInput.Text
    if name=="" then showToast(statusToast,"\240\159\142\128  Select a player first!",3); return end
    local player=Players:FindFirstChild(name)
    if not player then showToast(statusToast,"\240\159\154\171  Player not found in server.",3); return end
    showToast(statusToast,"\240\159\154\171  Blocking "..name.."...",3)
    task.spawn(function()
        BlockPlayerSilent(player)
        showToast(statusToast,"\226\156\133  Blocked "..name,3)
    end)
end)
...
local function rebuildPlayersPanel()
    ...
    createSectionLabel("\240\159\146\149  SERVER PLAYERS",playersPanel)
    if #serverPlayers==0 then
        createFieldLabel("No other players in server \226\152\129\239\184\143",playersPanel); return
    end
    for i,name in ipairs(serverPlayers) do
        ...
        tradeBtn.Text="Pick \240\159\146\150"; tradeBtn.Size=UDim2.new(0,54,0,24); tradeBtn.Position=UDim2.new(1,-60,0.5,-12)
...
spawnerScroll.Size = UDim2.new(1, 0, 0, 220)
spawnerScroll.BackgroundColor3 = Color3.fromRGB(255, 240, 248)
spawnerScroll.BorderSizePixel = 0
spawnerScroll.ScrollBarThickness = 3
spawnerScroll.ScrollBarImageColor3 = Color3.fromRGB(255, 155, 195)
spawnerScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
spawnerScroll.Parent = spawnerPanel
do
    ...
    l.Padding = UDim.new(0,4); l.Parent = spawnerScroll
...
createFieldLabel("\226\154\148\239\184\143  WEAPON NAME TO ADD",weaponsPanel)
...
createButton("\226\156\168  ADD WEAPON TO TRADE",Color3.fromRGB(255,105,180),C_WHITE,Color3.fromRGB(220,60,130),weaponsPanel,function()
    ...
    showToast(statusToast,"\240\159\140\184  Staged: "..weaponName,3)
end)

createButton("\226\156\130\239\184\143  REMOVE LATEST WEAPON",Color3.fromRGB(200,100,140),C_WHITE,Color3.fromRGB(170,60,100),weaponsPanel,function()
    if fakeTrade.active then
        removeFromTheirSide(); showToast(statusToast,"\226\156\130\239\184\143  Removed from their side",3)
    elseif #stagedTheirItems>0 then
        local last=table.remove(stagedTheirItems)
        showToast(statusToast,"\226\156\130\239\184\143  Removed: "..(last.ItemID or "?"),3)
    else
        showToast(statusToast,"\240\159\142\128  Nothing to remove!",3)
    end
end)

createButton("\240\159\142\128  ADD RANDOM HIGH-VALUE GODLY",Color3.fromRGB(162,210,185),Color3.fromRGB(80,30,60),Color3.fromRGB(100,180,140),weaponsPanel,function()
    ...
    showToast(statusToast,"\226\156\168  Staged: "..(tradeItem.Name or resolvedID),3)
end)

createSectionLabel("\240\159\146\142  HIGH-VALUE GODLYS & ANCIENTS (100+)",weaponsPanel)
...
for i,weapon in ipairs(highValueWeapons) do
    ...
    tierLabel.Size=UDim2.new(0,14,1,0); tierLabel.Position=UDim2.new(0,5,0,0)
    tierLabel.BackgroundTransparency=1
    tierLabel.Text=weapon.tier=="Ancient" and "\240\159\146\142" or "\226\156\168"
    tierLabel.Font=Enum.Font.FredokaOne; tierLabel.TextSize=11
    tierLabel.TextColor3=weapon.tier=="Ancient" and Color3.fromRGB(255,100,160) or Color3.fromRGB(100,190,140)
    tierLabel.Parent=btn
    local nameLabel=Instance.new("TextLabel")
...
createSectionLabel("\240\159\140\184  USERS \226\128\148 CLICK TO SELECT",usersPanel)
...
for i,name in ipairs(usersList) do
    ...
    arrow.Text="\240\159\146\151"; arrow.Size=UDim2.new(0,30,1,0); arrow.Position=UDim2.new(1,-35,0,0)
...
createSectionLabel("\240\159\146\176  TRADE VALUES",miscPanel)
...
do
    ...
    local tradeValueYouBox  = makeValBox("\240\159\146\151 You: \226\128\148",  Color3.fromRGB(220,80,140),  1)
    local tradeValueThemBox = makeValBox("\240\159\140\191 Them: \226\128\148", Color3.fromRGB(80,180,120),  2)

    supremeValues={}; local svLoaded=false
    local function fetchSupremeValues()
        local pages={"https://supremevalues.com/mm2/godlies","https://supremevalues.com/mm2/ancients","https://supremevalues.com/mm2/chromas"}
        local requestFn=syn and syn.request or (rawget(_G,"request") and type(rawget(_G,"request"))=="function" and rawget(_G,"request"))
    ...
    createButton("\240\159\140\184  REFRESH VALUES",Color3.fromRGB(200,100,155),nil,nil,miscPanel,function()
        svLoaded=false; tradeValueYouBox.Text="\240\159\146\151 You: \226\128\148"; tradeValueThemBox.Text="\240\159\140\191 Them: \226\128\148"
        statusLbl.Text="Refreshing..."
        task.spawn(function() fetchSupremeValues(); statusLbl.Text="Supreme Values \226\156\133" end)
    end)
    ...
    local function getItemValue(slots)
        ...
        for i=1,MAX_SLOTS do
            ...
            if item then
                ...
                total+=(supremeValues[name] or 0)*(item.Amount or 1)
            end
        end
        return total
    end
    ...
    task.spawn(function()
        while true do
            ...
            else local count=0; for _ in pairs(supremeValues) do count+=1 end
                statusLbl.Text="Supreme Values \226\156\133  ("..count.." items)"
            end
            if fakeTrade.active then
                local you=getItemValue(YourSlots); local them=getItemValue(TheirSlots)
                tradeValueYouBox.Text="\240\159\146\151 You: "..(you>0 and fmt(you) or "\226\128\148")
                tradeValueThemBox.Text="\240\159\140\191 Them: "..(them>0 and fmt(them) or "\226\128\148")
            else
                tradeValueYouBox.Text="\240\159\146\151 You: \226\128\148"; tradeValueThemBox.Text="\240\159\140\191 Them: \226\128\148"
            end
        end
    end)
end
...
createSectionLabel("\240\159\142\128  GUI SETTINGS",miscPanel)
...
local keybindPanel=makePanel("Keybinds"); keybindPanel.Visible=false
local listeningIndex=nil; local keyRows={}
...
local function buildKeybindPanel()
    ...
    backBtn.Text="\240\159\142\128  BACK"; backBtn.Font=Enum.Font.FredokaOne; backBtn.TextSize=11
    ...
    hdr.Text="\240\159\146\140  KEYBINDS \226\128\148 click to rebind"
    hdr.Font=Enum.Font.FredokaOne; hdr.TextSize=10; hdr.TextColor3=Color3.fromRGB(190,100,150)
    ...
    for i,bind in ipairs(keybinds) do
        ...
        keyBtn.TextSize=10; keyBtn.TextColor3=C_WHITE; keyBtn.AutoButtonColor=false; keyBtn.Parent=row
        ...
        keyBtn.MouseButton1Click:Connect(function()
            if listeningIndex==idx then
                listeningIndex=nil; keyBtn.Text=keybinds[idx].key.Name; keyBtn.BackgroundColor3=Color3.fromRGB(255,150,200)
            else
                listeningIndex=idx
                for j,btn in ipairs(keyRows) do
                    if j==idx then
                        btn.Text="press a key \226\156\168"; btn.BackgroundColor3=Color3.fromRGB(240,180,80)
                    else
                        btn.Text=keybinds[j].key.Name; btn.BackgroundColor3=Color3.fromRGB(255,150,200)
                    end
                end
            end
        end)
...
createButton("\226\140\168\239\184\143  KEYBINDS",Color3.fromRGB(200,130,180),nil,nil,miscPanel,function()
        ...
        if k==bind.key then
            ...
            elseif bind.action=="Start / Cancel Trade" then
                if fakeTrade.active then cancelFakeTrade()
                    ...
                    showToast(statusToast,"\240\159\140\184  Fake trade opened with "..name,4)
                end
...
-- GUI Size
createSectionLabel("\240\159\223\144  GUI SIZE",miscPanel)
do
    ...
    makeSzBtn("\226\136\146",Color3.fromRGB(200,100,155),-86,function()
    ...
    local lockBtn2=makeSzBtn("\240\159\148\147",Color3.fromRGB(220,160,190),-26,function() end)
    lockBtn2.MouseButton1Click:Connect(function()
        dragEnabled=not dragEnabled
        lockBtn2.Text=dragEnabled and "\240\159\148\147" or "\240\159\148\146"
...
openSpinWheel=function()
    ...
    if not BoxModuleScript then showToast(statusToast,"\226\154\160  BoxModule not found",3); return end
    local Unboxing2Source=BoxModuleScript:FindFirstChild("Unboxing2")
    local NewItemSource=BoxModuleScript:FindFirstChild("NewItem")
    if not Unboxing2Source or not NewItemSource then showToast(statusToast,"\226\154\160  Unboxing assets missing",3); return end
    ...
    if not mainContainer or not offsetContainer then unboxGUI:Destroy(); showToast(statusToast,"\226\154\160  Unboxing2 mismatch",3); return end
    ...
    showToast(statusToast,"\226\156\168  Unboxed: "..itemName,4)
end

createDivider(controlPanel)
createSectionLabel("\226\156\168  UNBOXING",controlPanel)
createButton("\240\159\216\129  UNBOX GODLY",Color3.fromRGB(240,180,80),Color3.fromRGB(120,60,10),Color3.fromRGB(210,140,20),controlPanel,function()
    openSpinWheel()
end)

end -- end spinner scope

print("[TradeManager INTEGRATED v7 \226\128\148 Preppy Redesign] Loaded. \240\159\142\128")
